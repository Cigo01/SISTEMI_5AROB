...
while len(unexploredNode) > 0:  #ciclo fino a quando non esploro tutti i nodi

    print("Passo numero: %d" % cntPasso)
    print("Nodi inesplorati: ", unexploredNode)
    print("Lista delle label: ", labelList)

    infiniteList = [math.inf] * len(graph) # scelgo il nodo con label minore
    ...
    for node, weight in enumerate(
            graph[currentNode]):  # ciclo for con ricerca dei nodi collegati a k per indici non ancora esplorati
        ...
        if (weight != 0):  # verifico che sia collegato e che non sia se stesso
            ...
            if (newLabel < labelList[node]):
                labelList[
                    node] = newLabel  # per ogni nuovo nodo sommo (valore label(k)+peso arco di quel nodo) il valore della sua label e controllo se sono minori così sostituisco sennò niente
